...
- Tests: determinism per arrangement; every arrangement × 10 seeds → all cells resolved, palette law (program mode) holds, quilt-pass ≥ 60% per arrangement *(AMENDED at execution: the test asserts a 50% sanity floor; rates are measured honestly and the Task 4 visual gate decides shipping — column-short shipped flagged at 0.50)* (report per-arrangement rates — the gate decides shipping); renderer emits correct canvas dims; composition metrics computed on all sizes.
