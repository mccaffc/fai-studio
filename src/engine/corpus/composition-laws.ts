/**
 * composition-laws.ts - shared measurement definitions for P10 composition laws.
 *
 * This module is intentionally pure engine code. The canon CLI supplies mined
 * tile families from the manifest; sampler tests supply families from the baked
 * tile catalog. The definitions below are the Task 1 definitions used for both.
 */

import type { BannerPlan, CellPlan, FormGroup } from './types.js';

export const RHYTHM_TEMPLATE_IDS = ['checker-motif', 'repeat-rhythm'] as const;
export type RhythmTemplateId = typeof RHYTHM_TEMPLATE_IDS[number];

export const NEUTRAL_FILLS = ['#121212', '#D9D9D6', '#F3F3F3', '#FFFFFF'] as const;

const NEUTRAL_FILL_SET = new Set<string>(NEUTRAL_FILLS);
const THIRD_POINTS = [
...
export interface CompositionLawMeasurements {
  ...
  source: {
    ...
    bannerCount: number;
    neutralFills: string[];
    rhythmTemplates: RhythmTemplateId[];
  };
  definitions: typeof COMPOSITION_LAW_DEFINITIONS;
...
export const COMPOSITION_LAW_DEFINITIONS = {
...
interface MeasureOptions {
  familyForTile: (tile: string) => string | undefined;
  templateIdForPlan?: (plan: BannerPlan) => string | null;
}

export function measureCompositionLawPlans(
  plans: readonly BannerPlan[],
  options: MeasureOptions & { corpusMinedAt: string },
): CompositionLawMeasurements {
  const banners = plans.map(plan => measurePlanComposition(plan, options));
  return {
    schemaVersion: 1,
    source: {
      corpusPath: 'corpus/corpus.json',
      corpusMinedAt: options.corpusMinedAt,
      bannerCount: plans.length,
      neutralFills: [...NEUTRAL_FILLS].sort(compareCodepoint),
      rhythmTemplates: [...RHYTHM_TEMPLATE_IDS],
    },
    definitions: COMPOSITION_LAW_DEFINITIONS,
    banners,
    aggregates: aggregateCompositionMeasurements(banners),
  };
}

export function measurePlanComposition(
  plan: BannerPlan,
  options: MeasureOptions,
): BannerCompositionMeasurements {
  const templateId = options.templateIdForPlan?.(plan) ?? plan.templateId ?? null;
  return {
    id: plan.id,
    templateId,
    accentProximity: measureAccentProximity(plan),
    focalPosition: measureFocalPosition(plan, options.familyForTile),
    rhythmBreak: isRhythmTemplate(templateId) ? measureRhythmBreak(plan, templateId) : null,
  };
}

export function aggregateCompositionMeasurements(
  banners: readonly BannerCompositionMeasurements[],
): CompositionLawAggregates {
...
export function accentFillsForCell(cell: Pick<CellPlan, 'ground' | 'ink' | 'inks'>): string[] {
  const fills = new Set<string>();
  for (const fill of [cell.ground, cell.ink, ...(cell.inks ?? [])]) {
    if (!fill) continue;
    const normalized = fill.toUpperCase();
    if (!NEUTRAL_FILL_SET.has(normalized)) {
...
export function rhythmSignature(cell: CellPlan): string {
  const tile = cell.kind === 'tile' ? (cell.tile ?? 'missing') : '-';
  const ink = cell.ink ?? '-';
  return `${cell.kind}|${tile}|${ink}|${cell.ground}`;
}

function measureAccentProximity(plan: BannerPlan): AccentProximityMeasurement {
  const byPosition = cellsByPosition(plan);
  ...
  for (const [startKey, start] of [...nodes.entries()].sort(([a], [b]) => comparePositionKey(a, b))) {
    ...
    while (stack.length > 0) {
      ...
      for (const neighbor of accentNeighbors(node.cell, plan, nodes, byPosition)) {
  ...
  const isolatedAccentCells = [...nodes.values()]
    .filter(node => accentNeighbors(node.cell, plan, nodes, byPosition).length === 0)
    .map(node => nearestAccentSingleton(node, [...nodes.values()], plan))
...
function measureFocalPosition(
  plan: BannerPlan,
  familyForTile: (tile: string) => string | undefined,
): FocalPositionMeasurement | null {
  const candidates = [
    ...formFocalCandidates(plan),
    ...signatureRegionFocalCandidates(plan, familyForTile),
  ];
  ...
  const centroid = focalCentroid(cells, plan);
  const centroidCell = centroidContainingCell(centroid, plan);
  ...
  return {
    ...
    centroid,
    centroidCell,
    inCenterCell: isCenterCell(centroidCell, plan),
...
function measureRhythmBreak(
  ...
  for (let row = 0; row < plan.rows; row += 1) {
    ...
    for (let col = 0; col < plan.cols; col += 1) {
      ...
      if (!cell) throw new Error(`plan ${plan.id}: missing cell ${col},${row}`);
      cells.push(cell);
    }
...
function formFocalCandidates(plan: BannerPlan): FocalCandidate[] {
  return plan.forms
...
function signatureRegionFocalCandidates(
  plan: BannerPlan,
  familyForTile: (tile: string) => string | undefined,
): FocalCandidate[] {
  const byPosition = cellsByPosition(plan);
  ...
  for (const cell of plan.cells) {
    const signature = focalSignature(cell, familyForTile);
    if (signature) {
  ...
  for (const [startKey, startSignature] of [...signatureByPosition.entries()].sort(([a], [b]) => comparePositionKey(a, b))) {
    ...
    while (stack.length > 0) {
      ...
      for (const [nextCol, nextRow] of neighbors8(cell.col, cell.row)) {
        if (nextCol < 0 || nextCol >= plan.cols || nextRow < 0 || nextRow >= plan.rows) continue;
...
function classifyRhythmLine(cells: CellPlan[], axis: Axis, index: number): RhythmLineMeasurement {
...
function nearestAccentSingleton(node: AccentNode, nodes: AccentNode[], plan: BannerPlan): IsolatedAccentCell {
  ...
  return {
    ...
    corner: isCorner(node.cell, plan),
  };
}
...
function accentNeighbors(
  ...
  byPosition: Map<string, CellPlan>,
...
function focalSignature(cell: CellPlan, familyForTile: (tile: string) => string | undefined): string | null {
  if (cell.kind === 'plain' || cell.kind === 'review') {
    return null;
  }
  const fill = cell.ink ?? firstForegroundInk(cell) ?? cell.ground;
  if (cell.kind === 'tile' && cell.tile) {
    const family = familyForTile(cell.tile) ?? 'unknown';
...
function firstForegroundInk(cell: CellPlan): string | null {
  return cell.inks?.[0] ?? null;
}

function focalCentroid(cells: Position[], plan: BannerPlan): { x: number; y: number } {
  const x = cells.reduce((total, cell) => total + cell.col + 0.5, 0) / cells.length / plan.cols;
  const y = cells.reduce((total, cell) => total + cell.row + 0.5, 0) / cells.length / plan.rows;
  return { x: round(x), y: round(y) };
}

function centroidContainingCell(centroid: { x: number; y: number }, plan: BannerPlan): Position {
  return {
    col: clamp(Math.floor(centroid.x * plan.cols), 0, plan.cols - 1),
    row: clamp(Math.floor(centroid.y * plan.rows), 0, plan.rows - 1),
  };
}
...
function interruptionPosition(axis: Axis, cell: CellPlan, lineLength: number): InterruptionPosition {
  const index = axis === 'row' ? cell.col : cell.row;
  return index === 0 || index === lineLength - 1 ? 'edge' : 'interior';
}

function isCenterCell(cell: Position, plan: BannerPlan): boolean {
  const centerCols = centerIndices(plan.cols);
  const centerRows = centerIndices(plan.rows);
...
function isCorner(cell: Position, plan: BannerPlan): boolean {
  return (cell.col === 0 || cell.col === plan.cols - 1) && (cell.row === 0 || cell.row === plan.rows - 1);
}
...
function cellsByPosition(plan: BannerPlan): Map<string, CellPlan> {
...
function isRhythmTemplate(templateId: string | null): templateId is RhythmTemplateId {
