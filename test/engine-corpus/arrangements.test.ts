 ...
 *
 * Locks the default banner sampler against a pre-change fixture, then exercises
 * every supported arrangement across sampling, rendering, scoring, program
 * palettes, and degenerate grid rules.
 */
...
function baselineFixture(): unknown {
  const path = join(process.cwd(), 'test', 'engine-corpus', 'fixtures', 'sample-plan-42-banner.json');
...
describe('banner identity fixture', () => {
  it('samplePlan(GRAMMAR, 42) without arrangement equals the pre-change banner fixture', () => {
    expect(samplePlan(GRAMMAR, 42)).toEqual(baselineFixture());
  });
});
